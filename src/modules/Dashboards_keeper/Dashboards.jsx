...
function Dashboards(props){
  const ipBoard = createContext(props.ipBoard)
  const port = createContext(props.port)
      ...
      <>
        ...
        <div className="dashboards">
          <div>
            ...
            <p className="value">Наибольшее граничное значение потльзовательской температуры: {allValues[allValues.length-1].temperature}C</p>
            <p className={allValues[allValues.length-1].errors[7] === '1' || allValues[allValues.length-1].errors[1] ==='1' ?
            'error-text' : 'no error-text'} >
              {allValues[allValues.length-1].errors[7] === '1' ?
              'Пользовательская температура находится за пороговым значением' : allValues[allValues.length-1].errors[1] === '1' ?
